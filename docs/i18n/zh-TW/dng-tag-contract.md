...
`image2dng validate` 會檢查 embedded preview layout（若存在）、required raw tags、identity/version/compression tags、geometry、RawDataUniqueID digest、crop/scale raw area tags、black/white levels、camera profile tags、mode-specific CFA tags、XMP provenance/raw mode/numeric consistency，以及 MakerNote absence。
