...
若 external scene manifest 明確設定 `apply_semantic_reaction: true`，pipeline 會啟用 `semantic-reaction-chain-v1` helpers，目前先用 `region-exposure-mask-v1` 依 region mask 與 `exposure_bias_ev` 對 16-bit scene-linear RGB input 進行 deterministic EV modulation，再套用 `highlight-clipping-policy-v1` 的 highlight shoulder mapping。這只支援 `linear-rec709`、`acescg`、`xyz`，不支援 encoded `srgb` 或 `prophoto-rgb` 直接套用 reaction。詳細 contract 見 [Semantic Scene Sidecar Contract v1](semantic-scene-sidecar-contract.md)。
