# Image-to-DNG RAW Generator 設計總覽

English public baseline: [docs/design.md](../../design.md)
...
這會產生 scene-linear TIFF intermediates、LinearRaw DNG、simulated CFA DNG、JPEG previews、validation JSON 與 graph manifest。詳見 [docs/i18n/zh-TW/raw-native-node-pipeline.md](raw-native-node-pipeline.md) 與 [docs/i18n/en/raw-native-node-pipeline.md](../en/raw-native-node-pipeline.md)。
...
6. 加入 black level offset，clip 到 white level。
7. quantize 為 16-bit LinearRaw buffer。
